...
def load_prompt():
    prompt_path = os.path.join("ai_prompt", "govt_question_prompt.txt")
...
def push_to_supabase(questions_data, exam_id, subject_id, chapter_id):
    ...
    existing_res = supabase.table("py_questions").select("question").eq("chapter_id", chapter_id).execute()
    ...
    for q in questions_data:
        ...
        try:
            ...
            supabase.table("py_questions").insert(payload).execute()
            inserted_count += 1
...
def generate_questions(
    ...
    with open("outputs/raw_model_govt_response.txt", "w", encoding="utf-8") as rf:
        rf.write(response_text)

    questions = safe_json_parse(response_text)

    if not questions:
        print("No questions could be parsed. See outputs/raw_model_govt_response.txt. Exiting.")
        return
    ...
    with open("outputs/govt_generated_questions.json", "w", encoding="utf-8") as f:
